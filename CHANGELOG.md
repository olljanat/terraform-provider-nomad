...
IMPROVEMENTS:
* resource/nomad_namespace: add `enabled_network_modes` and `disabled_network_modes` attributes ([#x](https://github.com/hashicorp/terraform-provider-nomad/pull/x))
...
NOTES:
* This release includes the license file along with the provider binary in the released archive files ([#455](https://github.com/hashicorp/terraform-provider-nomad/pull/455))
...
* resource/nomad_csi_volume: changes to `capacity_min` or `capacity_max` may now expand the volume instead of forcing replacement, on Nomad version 1.6.3 or later, if the CSI plugin supports it ([#382](https://github.com/hashicorp/terraform-provider-nomad/pull/382))
